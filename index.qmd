...
Having set up all the necessary functions, I now simulate a trivariate random walk to produce artificial data.
...
N       = ncol(Y) #Number of variables in the model
...
#| warning: false
# Run Basic function
Basic = Gibbs.sampler.baseline(p=1,Y=Y,X=X,priors=priors,S1=200,S2=300, FF.V=FF.V, B0.initial=B0.initial)
Basic 

```

Since a trivariate random was simulated, the first column of $B_+$ should be zero and the matrix $B_+[,2:4]$ should be an identity matrix. However, this is not entirely the case, which indicates that there might be a bug in the code. This will be looked deeper into.

## Extended Model

As part of my extended model I will estimate the shrinkage parameters $\kappa_0$ and $\kappa_+$. Estimating those parameters instead of just setting them might lead to improved efficiency and reliability. By remembering how $\kappa_0$ and $\kappa_+$ affected the posterior parameters in the basic model we can now write up the kernel for the new conjugate-prior up for the extended model:
...
# Run Basic function
# Run function
extended = Gibbs.sampler.extended(p=1,Y=Y,X=X,priors=priors,S1=200,S2=300, FF.V=FF.V, B0.initial=B0.initial)
...
It is clear that $B_+[,2:4]$ still isn't an identity matrix just like with the basic model. This problem will be fixed in the upcoming future.

I now turn to plotting the diagonal elements of $B_+[,2:4]$ in order to show whether the algorithm converges. 
...
It is not entirely clear whether the algorithm converges, however this would most likely be the case for even more draws. Due to computational costs I haven't run it with "enough" draws yet.
